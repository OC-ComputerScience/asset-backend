...
// Retrieve all AssetTypes with a specific categoryId from the database including their associated AssetCategory.
exports.getAssetTypesByCategoryId = (req, res) => {
  const categoryId = req.params.categoryId; // Get categoryId from request parameters

  AssetType.findAll({
    where: { categoryId: categoryId }, // Filter AssetTypes by categoryId
    include: [
      {
        model: AssetCategory,
        as: "assetCategory",
    ...
    .then((data) => {
      if (data.length > 0) {
        ...
        res.status(404).send({
          message: `No AssetTypes found with categoryId=${categoryId}.`,
        });
      }
    })
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving asset types by category.",
      });
    });
};
